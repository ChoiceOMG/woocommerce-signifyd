...
class WC_Signifyd_Case_Builder {
	...
	public static function map_avs( $code ) {
		...
		if ( $code === '' ) {
			return array( 'U', __( 'AVS mapped from empty to U', 'fraud-screening-for-woocommerce-with-signifyd' ) );
		}
		...
		if ( isset( $map[ $code ] ) ) {
			return array(
				...
				sprintf(
					...
					__( 'AVS mapped from %1$s to %2$s', 'fraud-screening-for-woocommerce-with-signifyd' ),
					$code,
					$map[ $code ]
				),
			);
		...
		return array( $code, __( 'AVS mapping not required', 'fraud-screening-for-woocommerce-with-signifyd' ) );
	}
